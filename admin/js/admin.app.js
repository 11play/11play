...
/* =========================================================
   ...
   - Load and render canonical Dashboard summary data
   - Initialize and refresh Admin feature modules
   - Coordinate refreshes after Admin actions
   - Clear sensitive module state after logout or access denial

   Sole Admin:
   casinobuzzbd@gmail.com

   Security:
   - Client checks control only the interface
   - Cloud Functions remain the final authorization authority
   - No Super Admin
   - No custom claim
...
(function initializeAdminApp(
    ...
    function setOperationalError(error) {
        state.error =
    ...
    function createAdminInitials(admin) {
        const displayName =
            toSafeString(
                admin?.displayName
            );
    ...
    function normalizeDashboardSummary(result) {
        const source =
            isPlainObject(
                result?.summary
            )
            ...
            withdrawals: {
                ...
                approved:
                    toNonNegativeInteger(
                        ...
                        withdrawals.successful ??
                        source.approvedWithdrawals
                    ),

                rejected:
                    toNonNegativeInteger(
                        withdrawals.rejected ??
                        source.rejectedWithdrawals
                    ),

                cancelled:
                    toNonNegativeInteger(
                        withdrawals.cancelled ??
                        withdrawals.canceled ??
                        source.cancelledWithdrawals
                    ),

                pendingAmount:
                    toNonNegativeInteger(
                        withdrawals.pendingAmount ??
                        withdrawals.processingAmount ??
    ...
    function renderSummary(summary) {
        ...
        setSummaryElements(
            "[data-admin-summary-referrals-total]",
            source.referrals.total
        );

        setSummaryElements(
            "[data-admin-summary-referrals-pending]",
            source.referrals.pending
        );

        setSummaryElements(
            "[data-admin-summary-referrals-qualified]",
            source.referrals.qualified
        );

        setSummaryElements(
            "[data-admin-summary-referrals-approved]",
            source.referrals.approved
        );

        setSummaryElements(
            "[data-admin-summary-referrals-rewarded]",
            source.referrals.rewarded
        );

        setSummaryElements(
            "[data-admin-summary-referrals-rejected]",
            source.referrals.rejected
        );

        /*
         * Temporary legacy selector compatibility.
         */

        setSummaryElements(
            "[data-admin-summary-referrals-observing]",
            source.referrals.pending
        );

        setSummaryElements(
            "[data-admin-summary-referrals-valid]",
            source.referrals.rewarded
        );

        setSummaryElements(
            "[data-admin-summary-referrals-invalid]",
            source.referrals.rejected
        );

        setSummaryElements(
            "[data-admin-summary-referrals-reward]",
            source.referrals.totalReward,
            "money"
        );

        setSummaryElements(
            "[data-admin-summary-withdrawals-total]",
            source.withdrawals.total
        );

        setSummaryElements(
            "[data-admin-summary-withdrawals-pending]",
            source.withdrawals.pending
        );

        setSummaryElements(
            "[data-admin-summary-withdrawals-approved]",
            source.withdrawals.approved
        );

        setSummaryElements(
            "[data-admin-summary-withdrawals-rejected]",
            source.withdrawals.rejected
        );

        setSummaryElements(
            "[data-admin-summary-withdrawals-cancelled]",
        ...
        setSummaryElements(
            "[data-admin-summary-withdrawals-processing]",
            source.withdrawals.pending
        );

        setSummaryElements(
            "[data-admin-summary-withdrawals-successful]",
            source.withdrawals.approved
        );

        setSummaryElements(
            "[data-admin-summary-withdrawals-pending-amount]",
            source.withdrawals.pendingAmount,
            "money"
        );

        setSummaryElements(
            "[data-admin-summary-wallet-available]",
            source.wallets
                .availableBalance,
            "money"
        );
        ...
        setSummaryElements(
            "[data-admin-summary-wallet-held]",
            source.wallets
                .heldBalance,
            "money"
        );
        ...
        setSummaryElements(
            "[data-admin-summary-wallet-earned]",
            source.wallets
                .totalEarned,
            "money"
        );
        ...
        setSummaryElements(
            "[data-admin-summary-wallet-withdrawn]",
            source.wallets
                .totalWithdrawn,
            "money"
        );
        ...
        setSummaryElements(
            "[data-admin-summary-transactions-total]",
    ...
    async function initializeRouteModule(route) {
        const routeModule =
    ...
    async function refreshRouteModule(
        ...
        /*
         * Existing feature modules load their first dataset
         * inside init(). Avoid an immediate duplicate request.
         */
        if (
            initializedNow &&
    ...
    function handleAdminDataChanged() {
        ...
        /*
         * Feature modules may already call this method before
         * dispatching their mutation event. The dedupe window
         * prevents a duplicate summary request.
         */
